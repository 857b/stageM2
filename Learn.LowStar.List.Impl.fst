module Learn.LowStar.List.Impl
...
let reverse_loop_body #a
  ...
  =
      ...
      let _,data,_ = sg_uncons sg_f in
    let cell : B.pointer (cell a) = it_f.(0ul) in
    it_f.(0ul) <- sg_next cell sg_f;
    ...
    it_r.(0ul) <- cell;
      ...
      let _ = sg_mkcons (G.reveal cell, G.reveal data) sg_r in
      G.hide (
      let sg_f', sg_r' = G.reveal (B.deref h' sgs) in
...
let length_loop_test a (l : Ghost.erased nat) h0 r p seg count
  ...
  = fun () -> not (B.is_null (sgn_entry (p.(0ul)) seg.(0ul)))
...
let length_loop_body a (l:Ghost.erased nat) h0 r
  ...
  =
      let seg0 = seg.(0ul) in
    p.(0ul) <- sg_next p.(0ul) (G.reveal seg0);
...
let push (#a : Type0) (r : HS.rid) (x : a) (p : t a) (sg : G.erased (list_seg a){p == entry sg}):
  ST (B.pointer (cell a))
    ...
    (ensures  fun h0 rp h1 ->
      ...
      rp == entry sg')
  =
    let rp = B.malloc r ({ data = x; next = p }) 1ul in
      let _ = sg_mkcons (rp, x) sg in
    rp
...
let rec initi (#a : Type0) (r : HS.rid) (lb ub : U32.t)
  ...
  = if U32.(lb <^ ub) then begin
       ...
       let p = B.malloc r ({ data = f lb; next = p' }) 1ul in
         let _ = sg_mkcons (p, f lb) (G.reveal sg') in
       (| p, G.hide (cons_seg (p, f lb) sg')|)
    end else
       (| B.null, G.hide (empty_list a) |)

(* --- index --- *)

let rec index (#a : Type0) (p : t a) (sg : G.erased (list_seg a))
...
let rec insert_aux (#a : Type) (r : HS.rid) (i : U32.t) (x : a)
  ...
  =   let h0 = ST.get () in
      let hd,hdd,tl = sg_uncons sg in
    if i = 1ul then
       let p_f = B.malloc r ({next = (p.(0ul)).next; data = x}) 1ul in
       p.(0ul) <- { p.(0ul) with next = p_f };
         let _ = sg_mkcons (G.reveal hd, G.reveal hdd) (sg_mkcons (p_f,x) tl) in
         let h1 = ST.get () in
         introduce exists p_f. insert_post r (U32.v i) x sg h0 (loc_seg sg) p p_f h1
              with p_f and ()
    else begin
      let next_p = G.hide ((B.deref h0 p).next) in
      insert_aux r U32.(i -^ 1ul) x ((p.(0ul)).next) tl;
      let h1 = ST.get () in
      eliminate exists p_f . (insert_post r (U32.v i - 1) x tl h0 (loc_seg tl) next_p p_f h1)
        ...
        with p_f and sg_mkcons_lem h1 (G.reveal hd, G.reveal hdd) (insert_seg (U32.v i - 1) (p_f, x) tl)
    end
...
let insert_post_lemma (#a : Type) (r : HS.rid) (i : nat) (x : a) (sg : list_seg a) (h0 : HS.mem)
  ...
  = sg_uncons_lem h0 sg;
    sg_mkcons_lem h1 (L.hd sg.segment) (insert_seg (i - 1) (p_f, x) (tail_seg sg))
...
let set (#a : Type) (i : U32.t) (x : a) (p : t a) (sg : G.erased (list_seg a))
  ...
  =
    let p' = forward i p sg in
      ...
      let hd,_,tl = sg_uncons sg1 in
    p'.(0ul) <- { p'.(0ul) with data = x };
      let _ = sg_mkcons (G.reveal hd, x) tl in
      let h1 = ST.get () in
...
let last_loop_body #a (sg : G.erased (list_nil a)) c_h0
  ...
  =
      ...
      let _ = sg_uncons sg0 in
    l_it.(0ul) <- sg_next (l_it.(0ul)) sg0; 
    l_k.(0ul)  <- k0 + 1;
      let Cons _ _ = (tail_seg sg0).segment in
  ...
  =
    ...
    let test () : Stack bool (requires fun h -> inv h) (ensures fun _ b h -> test_post b h)
      = let h = ST.get () in lemma_last_loop_inv_it_live sg h0 it k h;
        let _,_,tl = sg_uncons (splitAt_seg_bd (B.deref h k) sg)._2 in
        not (B.is_null (sgn_entry (it.(0ul).(0ul)).next (G.reveal tl)))
    in
    C.Loops.while #(fun h -> inv h) #test_post test (last_loop_body sg h0 it k);
      let h1 = ST.get () in
      lemma_last_loop_inv_it_live sg h0 it k h1;
      let _,_,tl = sg_uncons ((splitAt_seg_bd (B.deref h1 k) sg)._2) in
      assert (G.reveal tl == empty_list a);
...
let append (#a : Type) (p0 : t a) (sg0 : G.erased (list_nil a)) (p1 : t a) (sg1 : G.erased (list_seg a))
  ...
  =
      let h00 = ST.get () in
    if B.is_null (sgn_entry p0 sg0) then p1
    else begin
...
let test () : Stack unit (fun _ -> True) (fun _ _ _ -> True)
  = push_frame ();
    ...
    let _ = sg_mkcons (b, 42ul) (empty_list _) in
    assert (b == entry sg);
    assert (live_seg h sg);
    pop_frame ()
#pop-options
