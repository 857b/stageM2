...
let make_combinator
  ...
  = {
    ...
    repr_steel = (fun pre post c ->
                    let M.TCgen s sh pre pre_eq post post_eq = c in
                    make_combinator_steel ek s (f s sh) #pre pre_eq #post post_eq)
  }
...
val lc_to_spc_steel_t_steel
      ...
      (f : M.repr_steel_t ek a env (LV2M.res_env_f env csm prd)
...
let lc_to_spc_steel_t
  ...
  : M.spc_steel_t ek (lc_spec_r lc)
  = lc_to_spc_steel_t_steel mr.repr_tree lc
      (mr.repr_steel env (LV2M.res_env_f env csm prd) (LV2M.repr_M_of_LV lc))
