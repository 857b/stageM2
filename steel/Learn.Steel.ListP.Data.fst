...
let rec mlist0 (p : list_param) (entry : ref p.r) (len : nat) (exit : ref p.r)
  ...
  =
    ...
    else vrewrite ((p.cell entry).vp `vdep` mlist_cons_vp p entry (len-1) exit)
                  (mlist_cons_sel p entry (len-1) exit)

and mlist_cons_vp (p : list_param) (entry : ref p.r) (len' : nat) (exit : ref p.r) (c : t_of (p.cell entry).vp)
...
let mlist_case_S (p : list_param) (entry : ref p.r) (len : nat) (exit : ref p.r)
  : Lemma (mlist0 p entry (len+1) exit ==
           vrewrite ((p.cell entry).vp `vdep` mlist_cons_vp p entry len exit)
...
let mlist_sel_case_S (p : list_param) (r0 : ref p.r) (len : nat) (exit : ref p.r) (m : Mem.mem)
  : Lemma (requires Mem.interp (hp_of (mlist0 p r0 (len+1) exit)) m)
          (ensures  Mem.interp (hp_of (p.cell r0).vp) m /\
                   (let c = sel_of (p.cell r0).vp m in
  ...
  =
    ...
    interp_vdep_hp (p.cell r0).vp (mlist_cons_vp p r0 len exit) m;
    calc (==) {
      sel_of (mlist0 p r0 (len+1) exit) m;
    == {}
      vrewrite_sel ((p.cell r0).vp `vdep` mlist_cons_vp p r0 len exit)
                   (mlist_cons_sel p r0 len exit) m;
    == { vrewrite_sel_eq ((p.cell r0).vp `vdep` mlist_cons_vp p r0 len exit)
                         (mlist_cons_sel p r0 len exit) m }
       mlist_cons_sel p r0 len exit
         (let (|c, rs|) =
           ((sel_of ((p.cell r0).vp `vdep` mlist_cons_vp p r0 len exit) m)
            <: vdep_t (p.cell r0).vp (mlist_cons_vp p r0 len exit))
          in (|c, rs|));
    == { vdep_sel_eq (p.cell r0).vp (mlist_cons_vp p r0 len exit) m }
     (let c = sel_of (p.cell r0).vp m in
      mlist_cons_sel p r0 len exit
...
let intro_mlist_cons_lem (p : list_param) (r0 r1 : ref p.r) (len : nat) (exit : ref p.r) (m : Mem.mem)
  : Lemma (requires Mem.interp (hp_of ((p.cell r0).vp `star` mlist0 p r1 len exit)) m /\
